...
This repository includes a `render.yaml` [Render Blueprint](https://render.com/docs/infrastructure-as-code) that provisions:
- A **web service** running the FastAPI app in Docker.
- A **managed PostgreSQL** database wired automatically via `DATABASE_URL`.
...
4. Render reads `render.yaml` and shows a preview of the resources to create.
5. Fill in the required environment variable values when prompted:
   - **`MAPBOX_PUBLIC_TOKEN`** – Your Mapbox public access token from [mapbox.com](https://mapbox.com).
   - **`OPENAI_API_KEY`** – Your OpenAI API key (optional; only needed for AI features).
   - `SECRET_KEY` is auto-generated by Render.
6. Click **Apply** to deploy. Render builds the Docker image and starts the service.

### Enable PostGIS on the Render Database

After the database is created (first deploy), open a shell connection to it and run:

```sql
CREATE EXTENSION IF NOT EXISTS postgis;
```

You can do this from the Render dashboard: **PostgreSQL service** → **Connections** → copy the connection string, then use `psql`:

```bash
psql "$DATABASE_URL" -c "CREATE EXTENSION IF NOT EXISTS postgis;"
```

The application startup will fail with a `type "geometry" does not exist` error if PostGIS is not enabled before the first request.

### Persistent File Uploads

Render's filesystem is **ephemeral** – files written to `app/static/uploads/` are lost on each deploy or restart. For production you have two options:

1. **Render Disk** – Attach a [persistent disk](https://render.com/docs/disks) to the web service and mount it at `/app/app/static/uploads`.
2. **External object storage** – Store uploads in S3, Cloudflare R2, or similar. Update `app/api/attachments.py` to use the storage SDK of your choice.
